---
title: "How to use transportMatching"
output: 
  prettydoc::html_pretty:
    theme: architect
    highlight: github
vignette: >
  %\VignetteIndexEntry{How to use transportMatching}
...
In this vignette we will explain how the functions of the package are used in order to estimate a contingency table. We will work on the `eusilc` dataset of the `laeken` package.
...
Here we set up the sampling designs and define all the quantities we will need for the rest of the vignette. The sample are selected with simple random sampling without remplacement and the weights are equal to the inverse of the inclusion probabilities.
...
The statistical mathching is done by using the `otmatch` function. The estimation of the contingency table is calculated by extracting the `id1` units (respectively `id2` units) and by using the function `tapply` with the correct weights.
